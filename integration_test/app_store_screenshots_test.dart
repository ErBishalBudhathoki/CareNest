...
void main() {
  ...
  testWidgets('App Store Screenshots - Full Tour', (WidgetTester tester) async {
    ...
    await waitForWidget(tester, loginFinder,
        timeout: const Duration(seconds: 60), label: 'Login/Sign In button');
    ...
    final invoicesTab =
        find.textContaining(RegExp(r'Invoices', caseSensitive: false));
    ...
    final assignBtn =
        find.textContaining(RegExp(r'ASSIGN', caseSensitive: false));
    if (assignBtn.evaluate().isNotEmpty) {
      await tester.tap(assignBtn.first);
      await tester.pumpAndSettle();

      final harry =
          find.textContaining(RegExp(r'Harry James', caseSensitive: false));
      if (harry.evaluate().isNotEmpty) {
        ...
        final confirm =
            find.textContaining(RegExp(r'Confirm', caseSensitive: false));
    ...
    if (menuBtn.evaluate().isNotEmpty) {
      ...
      await tester.pumpAndSettle();
      final logoutBtn =
          find.textContaining(RegExp(r'Log Out|Logout', caseSensitive: false));
    ...
    if (textFields2.evaluate().length >= 2) {
      ...
      if (menuBtn.evaluate().isNotEmpty) {
        await tester.tap(menuBtn.first);
        await tester.pumpAndSettle();
        final logoutBtn = find
            .textContaining(RegExp(r'Log Out|Logout', caseSensitive: false));
        if (logoutBtn.evaluate().isNotEmpty) {
          await tester.tap(logoutBtn.first);
          for (int i = 0; i < 15; i++) {
            await tester.pump(const Duration(milliseconds: 500));
    ...
    if (textFields3.evaluate().length >= 2) {
      await tester.enterText(textFields3.at(0), 'client.seed.1@mailinator.com');
      ...
      final openChat = find
          .textContaining(RegExp(r'OPEN CHAT|Messages', caseSensitive: false));
