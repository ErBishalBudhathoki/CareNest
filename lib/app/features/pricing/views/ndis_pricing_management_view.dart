...
class _NdisPricingManagementViewState
  ...
  String? _extractOrganizationState(Map<String, dynamic>? response) {
    ...
    for (final candidate in candidateMaps) {
      ...
      final businessState = _normalizeStateCode(
        candidate['businessState']?.toString(),
      );
  ...
  Future<void> _loadNdisItems() async {
    try {
      ...
      log.severe(
        "Failed to load NDIS items in NdisPricingManagementView",
        e,
        s,
      );
      if (mounted) {
        ...
        _showSnackBar(
          AppLocalizations.of(context)!.failedToLoadNdisItems,
          isError: true,
        );
      }
    }
  ...
  Future<void> _loadPricingData() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _fallbackBaseRate = (fallbackRate != null && fallbackRate > 0)
              ? fallbackRate
              : null;
        });
      }
      ...
      final allItemNumbers = _allNdisItems
  ...
  Map<String, dynamic>? _getNdisMaxCapInfo(NDISItem item) {
    ...
    final labelledCaps = _resolveLabelledCaps(
      priceCaps['labelled'],
      targetState,
    );
    final p01Price =
    ...
    if (selectedPrice == null) {
      final maxByStatePrice = _resolveStatePrice(
        priceCaps['maxByState'],
        targetState,
      );
  ...
  void _togglePriceOverride(String itemNumber) {
    setState(() {
      ...
      if (_showPriceOverride[itemNumber] == true) {
        ...
        _isCustomPriceEnabled[itemNumber] = _hasItemLevelCustomPricing(
          _pricingData[itemNumber],
        );
      } else {
        // Dispose controller
  ...
  Future<void> _saveCustomPricing(NDISItem item) async {
    ...
    try {
      ...
      if (userEmail.isEmpty) {
        _showSnackBar(
          AppLocalizations.of(context)!.missingUserEmail,
          isError: true,
        );
        setState(() {
  ...
  Widget _buildInfoBanner() {
    return Container(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Container(width: 6, height: 82, color: _accentRed),
          Expanded(
            child: Padding(
              ...
              children: [
                Container(
                  ...
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Container(width: 6, height: 62, color: _accentRed),
                      Expanded(
                        child: Padding(
                      ...
                      Expanded(
                        child: _buildOverrideQuickButton(
                          label: isSaving
                              ...
                              : AppLocalizations.of(
                                  context,
                                )!.saveCustomPriceAction.toUpperCase(),
                          isPrimary: true,
                          onTap: isSaving
                              ? null
                              : () => _saveCustomPricing(item),
                        ),
                      ),
                      const SizedBox(width: 8),
                      Expanded(
