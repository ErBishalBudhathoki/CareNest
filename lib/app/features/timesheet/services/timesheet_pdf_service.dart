...
class TimesheetPdfService {
  Future<String> generateTimesheetPdf({
    ...
    entries.sort((a, b) {
      final dateA =
          a.workDate ??
          (a.shiftDate != null ? DateTime.tryParse(a.shiftDate!) : null) ??
          DateTime(0);
      final dateB =
          b.workDate ??
          (b.shiftDate != null ? DateTime.tryParse(b.shiftDate!) : null) ??
          DateTime(0);
    ...
    final fileName =
        'Timesheet_${periodFormat.format(startDate).replaceAll(',', '').replaceAll(' ', '_')}.pdf';
  ...
  pw.Widget _buildTimesheetTable(
    ...
    final data = entries.map((e) {
      ...
      final shiftTime = (start.isNotEmpty && end.isNotEmpty)
          ? '$start - $end'
          : 'N/A';
