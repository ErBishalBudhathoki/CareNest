...
final earningsRepositoryProvider = Provider<EarningsRepository>((ref) {
  return EarningsRepository(ApiMethod());
});
...
class EarningsRepository {
  ...
  Future<EarningsSummary> getEarningsSummary(String userEmail,
    ...
    final response = await _apiMethod.get('api/earnings/summary/$userEmail$query');

    if (response['success'] == true) {
      await _cache.cacheSummary(userEmail, response['data']);
      return EarningsSummary.fromJson(response['data']);
    } else {
      throw Exception(response['message'] ?? 'Failed to fetch earnings summary');
    }
  }
  ...
  Future<ProjectedEarnings> getProjectedEarnings(String userEmail,
    ...
    final response = await _apiMethod.get('api/earnings/projected/$userEmail$query');

    if (response['success'] == true) {
      return ProjectedEarnings.fromJson(response['data']);
    } else {
      throw Exception(response['message'] ?? 'Failed to fetch projected earnings');
    }
  }
  ...
  Future<EarningsPeriodHistory> getEarningsHistory(
    ...
    final response = await _apiMethod.get('api/earnings/history/$userEmail$query');

    if (response['success'] == true) {
      return EarningsPeriodHistory.fromJson(response['data']);
    } else {
      throw Exception(response['message'] ?? 'Failed to fetch earnings history');
    }
  }
  ...
  Future<void> setPayRate(
    ...
    if (classificationLevel != null) body['classificationLevel'] = classificationLevel;
