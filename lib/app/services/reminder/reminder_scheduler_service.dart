...
class ReminderSchedulerService {
  ...
  Future<void> scheduleShiftReminders({
    ...
    if (shiftStartTime.isBefore(now)) {
      debugPrint('ReminderSchedulerService: Skipping past appointment $appointmentId');
      return;
    }
    ...
    final List<_ReminderConfig> reminders = [
      _ReminderConfig(
        ...
        body: 'You have a shift with $clientName tomorrow at ${_formatTime(shiftStartTime)}',
        idSuffix: 'day',
      ),
      _ReminderConfig(
        offsetMinutes: 60, // 1 hour
        title: 'Shift in 1 Hour',
        body: 'Your shift with $clientName starts in 1 hour${clientAddress != null ? ' at $clientAddress' : ''}',
        idSuffix: 'hour',
      ),
      _ReminderConfig(
        offsetMinutes: 30, // 30 minutes
        title: 'Shift Starting Soon',
        body: 'Your shift with $clientName starts in 30 minutes. Time to head out!',
        idSuffix: '30min',
      ),
    ];
    ...
    for (final reminder in reminders) {
      ...
      if (reminderTime.isAfter(now)) {
        final notificationId = _generateNotificationId(appointmentId, reminder.idSuffix);
        ...
        debugPrint('ReminderSchedulerService: Scheduled ${reminder.idSuffix} reminder for $appointmentId at $reminderTime');
      }
    }
  ...
  Future<void> cancelShiftReminders(String appointmentId) async {
    ...
    debugPrint('ReminderSchedulerService: Cancelled reminders for $appointmentId');
  }

  /// Reschedule all reminders on app startup.
  /// This is needed because scheduled notifications may be cleared on device restart.
  Future<void> rescheduleAllReminders(List<Map<String, dynamic>> upcomingAppointments) async {
    await initialize();
    
    debugPrint('ReminderSchedulerService: Rescheduling ${upcomingAppointments.length} appointments');
    ...
    for (final appointment in upcomingAppointments) {
      try {
        ...
        debugPrint('ReminderSchedulerService: Error rescheduling appointment: $e');
      }
    }
  }
  ...
  Future<void> _createReminderChannels() async {
    ...
    if (androidPlugin != null) {
      ...
      debugPrint('ReminderSchedulerService: Created reminder notification channels');
    }
  }
  ...
  Future<void> _persistScheduledReminders(String appointmentId, List<String> ids) async {
    try {
  ...
  Future<void> _removePersistedReminders(String appointmentId) async {
    try {
      ...
      debugPrint('ReminderSchedulerService: Error removing persisted reminders: $e');
    }
  }
}
