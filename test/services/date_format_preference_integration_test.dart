...
void main() {
  group('Date format preference integration via providers', () {
    test(
      'dateParserServiceProvider reflects dmy by default and mdy after change',
      () async {
    ...
    test(
      'datePeriodServiceProvider uses parser preference in derivePeriodFromItems',
      () async {
        final fakePrefsDmy = _FakePrefs();
        await fakePrefsDmy.init();
        await fakePrefsDmy.saveDateFormatPreference('dmy');
        final containerDmy = ProviderContainer(
          overrides: [
        ...
        final periodDmy = svcDmy.derivePeriodFromItems([
          '01/02/2025',
          '02/02/2025',
        ]);
        ...
        final containerMdy = ProviderContainer(
          overrides: [
        ...
        final periodMdy = svcMdy.derivePeriodFromItems([
          '01/02/2025',
          '02/02/2025',
        ]);
        expect(
          periodMdy.start,
          DateTime(2024, 12, 30),
        ); // Monday of week containing 2025-01-02
        expect(
          periodMdy.end,
          DateTime(2025, 2, 2),
        ); // Sunday of week containing 2025-02-02
      },
    );
  });
}
